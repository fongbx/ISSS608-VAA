...
```{r}
pacman::p_load(tidyverse, lubridate, zoo, ggstatsplot, plotly, crosstalk, gglorenz, DescTools, wesanderson, rstatix, webshot2, png, gt, ggridges, gganimate)
```
...
## Distribution of Income and Expenses among different Demographic Profiles
                            ...
                            list(type = "dropdown",
                                 ...
                                 buttons = list(
                                   ...
                                   list(method = "update",
                                        ...
                                        label = "Education"),
                                   list(method = "update",
                                        args = list(list(y = list(combined$NetIncome)),
                                                    list(yaxis = list(title = "NetIncome"))),
                                        label = "NetIncome")
                                   )
                                 )
                            )
...
-   Income seems to differ across residents of different education levels. Particularly, the income of residents with Graduate degree seem to be the highest, followed by residents with Bachelors degree. Income of residents with education level of Low or High School or College seem to be around the same range.
...
> H0: Mean of income is equal across education levels
>
...
> H0: Mean of shelter expense is equal across household sizes
...
Last but not least, we also want to understand how the residents' income and expenses have changed over time and whether there is any concern of cost inflation without commensurate increase in wages. We can visualise this through ---
...
```{r}
combined2 <- participants %>% inner_join(financial_yearmth, by = "participantId")
combined2$yearmonth <- as.Date(combined2$yearmonth, format='01 %m %Y')

ggplot(data = combined2,
       aes(x = AdjustedShelter, y = householdSize, fill = after_stat(x))) +
  ...
  transition_time(financial_yearmth$yearmonth) +
