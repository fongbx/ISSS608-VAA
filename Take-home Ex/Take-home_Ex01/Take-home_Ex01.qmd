...
```{r}
pacman::p_load(tidyverse, zoo, patchwork, ggstatsplot, gglorenz, plotly, rstatix, ggridges, gt, png, wesanderson)
```
...
We can view the final merged dataset using `DT::datatable()` function available in base R that creates an interactive table for sorting and searching through the data.
...
## Joviality among different Demographic Profiles

Last but not least, we want to understand what factors are correlated with the residents' happiness level. We will first use an interactive chart to explore how joviality differs by the different demographic profiles.
